...
/*   Updated: 2025/11/01 17:24:21 by kclaudan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_rgb_and_textures_checker(t_game *game)
{
	...
	while (game->file[i])
	{
		if (ft_is_identifier(game->file[i]) == 2 && ft_rgb_checker(game->file[i], &valid_rgb_counter) == FALSE)
			return (ft_putstr_fd("Error: invalid rgb\n", 2), FALSE);
		else if (ft_is_identifier(game->file[i]) == 1 && ft_textures_checker(game->file[i], &valid_path_counter) == FALSE)
