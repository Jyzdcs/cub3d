...
// "check_map_closed_utils.c et fais lui include map_parser.h
// en declarant ses prototypes dans le fichier map_parser.h
...
// bounds de la map, si il y a une sortie de la map car ouverte, on set la data a 1
...
// On va d'abord check que le personnage est dans la map, dans le cas particulier
...
{
	if (player->x < 0 || player->y < 0 || player->y >= map->height || player->x >= (int)ft_strlen(map_copy[player->y]))
	{
		map->is_closed = 1;
		return (1);
	}
